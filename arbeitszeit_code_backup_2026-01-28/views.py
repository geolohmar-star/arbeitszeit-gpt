...
def get_zeitoptionen():
    """Generiert Zeitoptionen von 2:00-12:15 in 15-Min-Schritten"""
    zeitoptionen = []
    zeitoptionen.append({'value': 0, 'label': 'Frei'})
...
def vereinbarung_erstellen(request):
    ...
    # Zeitoptionen generieren: 2:00 - 12:15 in 15-Min-Schritten
    zeitoptionen = []
    for hour in range(2, 13):  # 2-12
        for minute in range(0, 60, 15):  # 0, 15, 30, 45
            if hour == 12 and minute > 15:  # Stopp bei 12:15
                break
            value = hour * 60 + minute  # In Minuten
            label = f"{hour:02d}:{minute:02d}"
            zeitoptionen.append({'value': value, 'label': label})
    ...
    if request.method == 'POST':
        ...
        if antragsart != 'beendigung' and request.POST.get('arbeitszeit_typ') == 'individuell':
            ...
            for tag in tage:
                zeitwert_str = request.POST.get(f"{prefix}_{tag}")

                if zeitwert_str:
                    if ":" in zeitwert_str:
                        # HH:MM → Minuten
                        stunden, minuten = map(int, zeitwert_str.split(":"))
                        zeitwert_minuten = stunden * 60 + minuten
                    else:
                        # schon Minuten (z.B. "480")
                        zeitwert_minuten = int(zeitwert_str)

                    Tagesarbeitszeit.objects.create(
                        vereinbarung=vereinbarung,
                        wochentag=tag,
                        zeitwert=zeitwert_minuten
                    )
...
def vereinbarung_detail(request, pk):
    ...
    # WICHTIG: Tagesarbeitszeiten laden!
    ...
    # DEBUG: Prüfe ob Daten vorhanden
    print(f"DEBUG: Vereinbarung #{pk} hat {tagesarbeitszeiten.count()} Tagesarbeitszeiten")
    for tag in tagesarbeitszeiten:
        print(f"  {tag.wochentag}: {tag.zeitwert} Min = {tag.formatierte_zeit}")
    ...
    context = {
        'vereinbarung': vereinbarung,
        'tagesarbeitszeiten': tagesarbeitszeiten,  # ← MUSS im Context sein!
        'historie': historie,
    }
